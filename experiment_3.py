...
for data_iter in range(D):
    ...
    for n in range(START,DATA_MAX, INTERVAL): 
        ...
        home_data=home_data.sample(n+M)
        y=home_data.SalePrice        
        X=home_data[np.concatenate((feature_names_u,feature_names_v))]
...
#TotRmsAbvGrd: Total rooms above grade (does not include bathrooms)
        ...
        for j in range(M):
            ...
            for K_tilde in range(1,K_max+1):
                ...
                error_vb_K_tilde[K_tilde]=error_vb_K_tilde[K_tilde]+((y_exp_vb_K_tilde[j,K_tilde]*variance_y+mean_y-y_new[j])**2)/M
